...
const ReusablePageComponent = (props) => {
  ...
  const handleSelectionChange = (selectionModel) => {
    ...
    if(selectionModel.length>0){
      console.log('selected ware house', selectionModel, setSelectedWarehouse, typeof setSelectedWarehouse)
      setSelectedWarehouse(selectionModel[0])
  ...
  return (
    <div className="bg-[#FAFAF8] h-screen">
      <div className="section-header" style={{ marginTop: "30px" }}>
        <div>
          <p className="ml-8 mt-10 text-light text-4"><label style={{color: '#70707060'}}>Home/</label>{heading}</p>
                ...
                buttons: [
                  {
                    ...
                    onClick: () => {
                      callApi({
                        method: 'DELETE',
                        endpoint: "api/warehouses/"+selectedItems[0],
                        alert: true
                      }).then(()=>{
                        setTimeout(()=>{
                          props.getData();
